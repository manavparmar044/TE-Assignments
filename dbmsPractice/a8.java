    ...
    public static void main(String args[]){
        ...
        try(Connection con = DriverManager.getConnection(url,username,password);Scanner sc = new Scanner(System.in)){
            ...
            do{
                ...
                switch(choice){
                    case 1->{
                        try(Statement st = con.createStatement();ResultSet rs = st.executeQuery("SELECT * FROM emp_table")){
                            System.out.println("| ID |      Name      |  Position  |  Salary  |");
                            while(rs.next()){
                                System.out.printf("%d %s %s %.2f%n", rs.getInt("emp_id"),rs.getString("emp_name"),rs.getString("emp_position"),rs.getDouble("emp_salary"));
                            }
                        }
                    }
                    case 2 -> {
